...
#/
    ...
    if ( b_per_zone )
        n_capture_zombies_needed = n_capture_zombies_needed_per_zone;  // GEN-ZC-01 fix: was b_capture_zombies_needed (dead write; per-zone limit never applied)
...
get_unclaimed_attack_points_between_indicies( n_start, n_end )
{
    ...
    for ( i = n_start; i <= n_end; i++ )  // GEN-ZC-03 fix: was i < n_end (excluded last index of each pillar group)
    {
...
set_recapture_zombie_attack_target( s_recapture_target_zone )
{
    ...
    foreach ( zombie in level.zone_capture.recapture_zombies )
    {
        ...
        zombie.ignore_player = [];  // GEN-ZC-02 fix: clear stale player-proximity state from previous generator
    }
}
