...
onplayerconnect()
{
    ...
    self thread watch_tank_runover_disconnect();  // TANK-EL-01 fix: clean up run-over linker on disconnect
}
...
tank_push_player_off_edge( trig )
{
    ...
    self endon( "disconnect" );  // TANK-TL-01 fix: prevent thread leak if player disconnects while on tank
...
zombies_watch_tank()
{
    ...
    // The old loop iterated the full zombie array every network frame to assign
    // tank_zombie_think() threads; after all zombies were initialized it did
    // nothing but still paid the traversal cost every frame at high rounds.
...
tank_flamethrower_get_targets( str_tag, n_flamethrower_id )
{
    ...
    foreach ( ai_zombie in a_zombies )
    {
        ...
        n_dot = vectordot( v_tag_fwd, v_to_zombie );  // TANK-MI-01 fix: was ai_zombie.origin (raw position), making cone check meaningless
...
enemy_location_override()
{
    ...
    if ( isdefined( self.tank_state ) )
    {
        ...
        if ( self.tank_state == "tank_chase" || self.tank_state == "none" && ( isdefined( enemy.b_already_on_tank ) && enemy.b_already_on_tank ) )
        {
            ...
            front_dist = distance2dsquared( enemy.origin, tank_front );  // TANK-MI-02 fix: was level.vh_tank.origin (identical to back_dist; zombies always routed to back)
