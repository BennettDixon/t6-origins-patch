...
_kill_zombie_network_safe_internal( e_attacker, str_weapon )
{
    ...
    self dodamage( self.health + 128, self.origin, e_attacker, e_attacker, "none", self.kill_damagetype, 0, str_weapon );
}
...
do_damage_network_safe( e_attacker, n_amount, str_weapon, str_mod )
{
    ...
    {
        // MI-09: log when a staff-kill call ends up in the partial-damage branch.
        // n_amount was set to self.health by the caller — if it is now less than
        // self.health, either health changed between the two reads or a float/int
        // representation mismatch caused the values to diverge.
        if ( isdefined( level._hrp_mi09_diag ) )
        {
            n_gap = self.health - n_amount;
            logprint( "[HRP MI-09] partial-damage branch: n_amount=" + n_amount
                + " self.health=" + self.health + " gap=" + n_gap
                + " weapon=" + str_weapon + "\n" );
            if ( !isdefined( level._hrp_mi09_partial ) )
                level._hrp_mi09_partial = 0;
            level._hrp_mi09_partial++;
        }
    ...
    else
    {
        if ( isdefined( level._hrp_mi09_diag ) )
        {
            if ( !isdefined( level._hrp_mi09_kill ) )
                level._hrp_mi09_kill = 0;
            level._hrp_mi09_kill++;
        }
