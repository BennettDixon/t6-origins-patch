...
lerp( chunk )
{
    ...
    // EL-01 fix: store link on self so the death watchdog in zm_patch_entity_leaks.gsc /
    // zm_highround_patch.gsc can delete it if the zombie is killed while this thread is
    // blocked at waittill_multiple.  Without this, link is a local variable that becomes
    // unreachable when the engine terminates this thread on entity death, leaking one
    // script_origin per zombie killed mid-window-attack.
    self._lerp_link = link;
