    ...
    if ( a_zombies.size )
    {
        for ( i = 0; i < a_zombies.size; i++ )
        {
            if ( isalive( a_zombies[i] ) )
            {
                ...
                {
                    if ( isdefined( level._hrp_mi06_diag ) && i > 0 )
                        level._hrp_mi06_saved++;  // fix redirected: zombie[0] was dead, used zombie[i]
                    self thread staff_air_zombie_source( a_zombies[i], str_weapon );  // MI-06 fix: was a_zombies[0], caused stale ref when closest zombie is dead
                }
                else
...
whirlwind_drag_zombie( e_whirlwind, str_weapon )
{
    ...
    else if ( isdefined( level._hrp_mi07_diag ) && isdefined( self ) && isalive( self ) )
    {
        // Whirlwind expired mid-drag — zombie survived with full health, now invisible.
        // This is the MI-07 invisible zombie reproduction path.
        if ( !isdefined( level._hrp_mi07_undamaged ) )
            level._hrp_mi07_undamaged = 0;
        level._hrp_mi07_undamaged++;
        logprint( "[HRP MI-07] undamaged zombie after drag: health=" + self.health + " origin=" + self.origin + "\n" );
    }
}
