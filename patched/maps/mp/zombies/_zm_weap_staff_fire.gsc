...
#/
    ...
    while ( n_alive_time > 0.0 )
    {
        ...
        foreach ( e_target in a_targets )
        {
            ...
            {
                if ( !is_true( e_target.is_on_fire ) )  // SA-10 fix: was self.is_on_fire (checked projectile, not target)
                    e_target thread flame_damage_fx( str_weapon, e_attacker );
                else if ( isdefined( level._hrp_sa10_diag ) )
                    level._hrp_sa10_blocked++;  // count redundant threads the dedup prevented
            }
        }
    }
